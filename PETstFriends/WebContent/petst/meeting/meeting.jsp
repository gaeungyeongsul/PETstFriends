	...
	$.ajax({
		...
		for(var io = 1; io<10; io++){
			...
			$('#listTable>tbody:last').append(
					//사진크기 400 / 350
				 "<td><table>"+ 
				 "<tr><td><div class=\"portfolio-item development\"><img src=\"img/"+proPic+"\" width=\"250px\" height=\"250px\"><div class=\"img-overlay2\"><div class=\"portfolio-text\">"+
		                    "<p  style=\"font-size:14px; color: white; text-align: left; padding-left: 30px\">모&nbsp임&nbsp명 : "+data.meetingBoard[number[io]].meetingBoard_title+
		                    "<br>모임 장소 : <br>&nbsp&nbsp&nbsp&nbsp"+place1
		                    +"<br>&nbsp&nbsp&nbsp&nbsp"+place2+"<br>모&nbsp임&nbsp일 : "+data.meetingBoard[number[io]].meetingBoard_startMeetingDate.substring(0,10)+
		                    "</p><a href=\"meetingview.do?meeting_boardno="+data.meetingBoard[number[io]].meeting_boardno+"\">click</a></div></div></div></td></tr></table></td>");				
// 							제목 23자 제한 걸어야함						
...
$(window).scroll(function(){
		...
		if($(window).scrollTop()+$(window).height()>$(document).height()-250&&stop==9){
			$.ajax({
				type : "get",
				url : "showMBC.do",
				dataType : "json",
...
<!--         <input type="button" value="글쓰기" onclick="location.href='writeForm.do'" style= "background-color:#CD853F; color:white; font-family: 'NanumSquareRound',sans-serif; font-size: 15px !important; font-weight:bold; height:40px; width: 80px; border:1; border-color:#CD853F; border-radius: 25px;" > -->
	...
	<input type="text" id="keyword" style="width: 300px;  right: 110px; height:28px; bottom: 90px; 
