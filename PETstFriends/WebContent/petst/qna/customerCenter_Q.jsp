								...
								.click(
										function() {
											...
											if (result) {
												...
												location.href = 'deleteOftenQnA.do?oftenQnA_boardno='
														+ boardno;
											} else {
											}
										})
					...
					<div class="col-md-8 col-sm-8"
						...
						<div style="border-bottom: 2px solid brown; width: 20%;">
							<div style="border-bottom: 5px solid #FFD232; width: 70%;">
								<div style="border: 1px solid white; height: 50px;">
									<h2 align="left">
										<font
											style="font-family: 'NanumSquareRound', sans-serif; font-size: 20px;">
											<b>자주하는 질문</b>
										</font>
									</h2>
								</div>
							...
							<table class="table" style="width: 800px;">
								...
								<tbody style="text-align: center;">
									<c:forEach items="${oqList }" var="oftenQnA" varStatus="status">
										<tr class="pointer"
											onclick="FaqToggleDetail( 'row_${status.count}' )">
											<td>${oftenQnA.oftenQnA_boardno }</td>
											<td width="50%;" style="text-align: left;">
												<%-- 								<a href="showOftenQnA.do?oftenQnA_boardno=${oftenQnA.oftenQnA_boardno }"> --%>
												${oftenQnA.oftenQnA_title } <!-- 										</a> -->
											</td>
											<td>관리자</td>
											<%-- 								${oftenQnA.oftenQnA_adminId } --%>
											<td><c:set var="str1"
													value="${oftenQnA.oftenQnA_writeDate }" />
												${fn:substringBefore(str1, '.0')}</td>
										</tr>
										<tr class="none" id="row_${status.count}">
											<td colspan="4">${oftenQnA.oftenQnA_content }<br> <c:if
													test="${adminCheck != null}">
													<button class="deleteBtn"
														value="${oftenQnA.oftenQnA_boardno }">삭제</button>
													<button
						...
						<div style="width: 800px;" align="right">
							<c:if test="${adminCheck != null}">
								<input type="button" value="글쓰기" id="writeBoard"
									onclick="location.href='writeOftenQnAForm.do'">
							</c:if>
						</div>
					</div>
				</div>
